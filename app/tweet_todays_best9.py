...
def fetch_game_links(date):
  ...
  game_date = soup_schedule.find('h1', class_='bb-head02__title').text # 9月24日（木）
  if (str(int(date[-5:-3])) not in game_date[:2]) or (str(int(date[-2:])) not in game_date[2:5]):
    return [] # 取得リンクの日付がリクエストの日付と異なっていた場合(オフシーズン対策)
...
def calc_batter_score(stats):
  ...
  eval_list = [0, 0, 4, 2.5, 0, 1.5, 0.5, 0.5, 1.5, -2, 1] # statsの第3要素以降と加重和をとる
...
def fetch_pitcher_stats(game_link):
  ...
  for row in pitcher_stats_rows:
    ...
    if stats_html:
      stats = list(map(lambda x: x.text, stats_html))[:14]
      if '\n' in stats[0]: # 勝負HSがどの選手にもついていない試合の場合（引き分けなど）
        stats.insert(0, '投')
        stats = stats[:14]
      else:
        stats[0] = '投'
...
def tweet(tweet_content):
  ...
  AS=os.environ['ACCESS_SECRET_TOKEN']
  # Twitterオブジェクトの生成
